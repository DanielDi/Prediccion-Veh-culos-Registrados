...
datos[,c(1,2,4,6, 7, 8)] <- sapply(datos[,c(1,2,4,6, 7, 8)], as.numeric)
...
df_train_ts$sin_season2 <- df_train_ts$sin_season1 - s1_modelo$predict(df_train_ts)
...
```{r}
acf(df_train_ts$sin_season2)
m = auto_corr(df_train_ts$Unidades, pacf = TRUE)
plot(m)
```
Resultado: p = 8 (porque tiene todo) y q = 7 
```{r}
R_modelo <-  arima(df_train_ts$sin_season2, order=c(8,0,8))
 # predicion para el an~o después

```
Predict final:
```{r}
pred_T <- predict(t_modelo, df_val)
pred_S1 <- s1_modelo$predict(df_val)
pred_S2 <- s2_modelo$predict(df_val)
pred_R <- predict(R_modelo, n.ahead = 365)
predict_final <- pred_T + pred_S1 + pred_S2 + as.vector(pred_R$pred)
predict_final[predict_final<0]<-0
plot(df_val$Ts, predict_final)
...
```{r}
# Quitar Fecha
datos_se2 <- datos_se[,-1]
datos_aux <- sample(2, nrow(datos_se2),
                   replace = T,
                   prob = c(0.75, 0.25))

df_train_se <- datos_se2[datos_aux == 1,]
df_test_se <- datos_se2[datos_aux == 2,]
...
lm__rmse <- rmse(df_test_se$Unidades, df_test_se$Y_predict)
...
```{r}
library(graphics)
plot(datos_se$Ts [120:150], df_test_se$Y_predict[120:150])
lines (datos_se$Ts[120:150], datos_se$Unidades[120:150], col ="red")
```
