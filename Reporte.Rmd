...
corrplot(corr_train)
```
E Box-plot de festivos (primero) y de domingos(segundo) muestra que en estos días el valor de unidades es aproximadamente cero;
```{r, echo=FALSE}
...
plot(datos$Ts [120:150], df_test$Y_predict[120:150])
...
Ahora creamos un modelo de "Box-Jenkins" que basa en interpretar los datos como una serie de tiempo.
En primer lugar, se sacan la tendencia (trend) de los datos mediante una regresión lineal:
```{r}
...
```
En el primer grafico se encuentra el modelo linear de la tendencia. En la segunda los datos sin la tendencia predictiva. También se pueden ver que hay un efecto sesional cada año. Esperamos que este efecto depende el mes. Podemos ver que especialmente en enero y deciembre hay efectos sesionales. 
```{r}
...
plot(mes_means)
```
Todavía podemos observar un efecto sesional, si miramos un año más preciso. Parece como una sesionalidad semanal. 
```{r}
...
plot(dia_means)
```
Ahora los datos sin los efectos sesionales, tienen todavía un impacto pequeño, sesional: Del mes. 
```{r}
...
```
Los datos parecen ahora estacionarios. Y finalmente podemos buscar una prdicción de la autocorelación. Para eso usamos un modelo arima. Los siguientes plots sirven para buscar buenas hiperparámetros del modelo arima. 
```{r}
...
```
Predición final: Al final calculamos la predicción para el año siguiente. Es importante que deshagamos los ajustes por tendencias y estacionalidad, si queremos observar las previsiones de los valores reales de las series del tiempo. Eso significa que añadimos los la predicción del arima, las sesionales y la de la tendencia. Dado que hemos observado que en días festivos y domingos el valor de las unidades siempre es cero o casi cero, predecimos por todos estos días cero. Así recibimos una prediccíon (negro) del modelo Box-Jenkins para las unidades reales (rojo).
```{r}
