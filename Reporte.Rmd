...
model_TS <- setRefClass("model_TS", fields = list(t = "lm", s1 = "seasonM", s2= "seasonW", s3 = "seasonD"), methods = list(
                       predict = function(data)
                       {
                         print(data$Festivo)
                         print((as.vector(data$Festivo == 1) | as.vector(data$Wday == 7)))
                         ...
                         data_trabajo$pred <- pred_T + pred_S1 + pred_S2 + pred_S3 + as.vector(pred_R$prod)
                         data_festivos$pred<-0
                         return (rbind(data_trabajo, data_festivos ))
                       }))

modelo_final <- new('model_TS', t = t_modelo, s1 = s1_modelo, s2 = s2_modelo, s3 = s3_modelo)
predict_final = modelo_final$predict(df_val)
plot(df_val$Ts, predict_final)
points(df_val$Ts, df_val$Unidades, col = "red")
df_val$pred_ts <-predict_final
df_val[as.vector(df_val$Festivo == 1) | as.vector(df_val$Wday == 7),]
```

```{r}


lm__mae <- mae(df_val$Unidades, df_val$pred_ts)
lm__mse <- mse(df_val$Unidades, df_val$pred_ts)
lm__rmse <- rmse(df_val$Unidades, df_val$pred_ts)
lm__rsq <- rsq(df_val, Unidades, pred_ts)
